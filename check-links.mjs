// check-links.mjs
//
// Guards the deploy surface for what the pages LINK TO, the way verify.mjs
// guards what they LOAD. carillon deploys its repo root (vercel.json
// outputDirectory ".") trimmed by .vercelignore, so "deployed output" here
// means: in the tree, and not excluded by that file.
//
...
//
// Usage: node check-links.mjs [dir]              (exit code 1 on any violation)
...
// Per-repo facts. outputDir mirrors vercel.json's outputDirectory (the repo
// root, so no build step assembles it); ignoreFile is what trims it;
// github.branch is the repo's default branch, where /blob/ links must point.
export const SITE = Object.freeze({
  repoRoot: HERE,
  outputDir: '.',
  build: null,
  ignoreFile: '.vercelignore',
  github: Object.freeze({ owner: 'jamessuuu', repo: 'carillon', branch: 'main' }),
});
...
export function main(site = SITE) {
  ...
  if (violations.length > 0) {
    console.error(`check-links: FAIL - ${violations.length} reference(s) would 404 on the deployed site`);
  ...
  console.log(
    `check-links: OK - every href/src/poster under ${site.outputDir}/ resolves inside the deployed output or to a tracked file on GitHub`,
  );
}
